  ...
  return (
    <div className="p-4 max-w-lg mx-auto space-y-6">
      ...
      <div className="space-y-2">
        <label className="text-sm text-muted-foreground font-medium">종목</label>
          ...
          {SPORT_TYPES.map((t) => (
            <button
              ...
              className={`flex-1 py-2 rounded-lg text-sm ${
                sportType === t.value ? 'bg-foreground text-background' : 'bg-muted border border-border/50'
              }`}
            >
              {t.label}
            </button>
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder="90"
            className="bg-muted border border-border/50 rounded-lg px-3 py-2 text-lg w-24"
          />
          <span className="text-muted-foreground/80">분</span>
        </div>
      </div>

      {/* RPE */}
      <div className="space-y-2">
        <label className="text-sm text-muted-foreground font-medium">RPE</label>
          ...
          {[3, 4, 5, 6, 7, 8, 9].map((v) => (
            <button
              ...
              className={`w-9 h-9 rounded-full text-sm ${
                rpe === v ? 'bg-foreground text-background' : 'bg-muted border border-border/50'
              }`}
            >
              {v}
            </button>

...
export default async function StatsPage() {
  ...
  return (
    <div className="p-4 max-w-lg mx-auto space-y-6">
          ...
          {configs.map((c) => (
            <div key={c.liftType} className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-white/30">{c.weekLabel}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* AMRAP 추이 */}
      <section className="space-y-2">
        <h2 className="text-sm text-white/60 font-medium">AMRAP 기록</h2>
            ...
            amrapLogs.slice(0, 10).map((set) => {
              ...
              return (
                <div key={set.id} className="flex items-center justify-between text-sm">
                  <span className="text-white/40">{d.getMonth() + 1}/{d.getDate()}</span>
      ...
      {/* 주간 러닝 */}
      <section className="space-y-2">
        <h2 className="text-sm text-white/60 font-medium">주간 러닝</h2>
              ...
              {weeklyRunEntries.map(([week, km]) => {
                ...
                return (
                  <div key={week} className="flex-1 flex flex-col items-center gap-1">
                    <span className="text-xs text-white/40">{km.toFixed(0)}</span>
                    <div
      ...
      {/* 체중 추이 */}
      <section className="space-y-2">
        <h2 className="text-sm text-white/60 font-medium">체중 추이</h2>
        <div className="bg-white/5 rounded-xl p-4 space-y-1">
          {bodyLogs.length === 0 ? (
            <p className="text-white/30 text-sm">아직 기록 없음</p>
          ) : (
